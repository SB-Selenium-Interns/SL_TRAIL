...
export const Launcapp = async (ENV_Type, ENV_Type1) => {
        ...
        if (DR == null || Ex_status.status == "LaunchNew") {
            console.log("Launching Browser1234");
            if (RC.Config_Data("Con", "BR_Name") == "FF") {
                DR = new Builder().forBrowser('firefox').build();
                DR.manage().window().maximize()
              }
                if (RC.Config_Data("Con", "BR_Name") == "Chrome") {
                    DR = new Builder().forBrowser('chrome').build();
                  }
                await DR.sleep(2000);
          }
        console.log("value of DR is", DR);
        try {
           await DR.findelement(By.css("button.ant-btn-lg:nth-child(1)"));
          }
        catch (error) {
            console.log("Opening Page1234");
            await DR.get(Url_path);
          }
    }